...
if(isset($_GET['pid']) && $_GET['pid']>0){
		...
		$dataArray=array(
					...
					'emp_status'					=>	trim(dbOutput($f['emp_status'])),
					'emp_code'						=>	trim(dbOutput($f['emp_code']))
...
$employee_territory=implode($_POST['employee_territory'],",");
	...
	$dataArray=array(
					...
					'emp_status'					=>	trim(dbOutput($_POST['emp_status'])),
...
//print_r($dataArray);
	...
	if(EmptyCheck($dataArray['username'])){
		 $errorArray['username']='Enter Username';
				...
				if(odbc_num_rows($rs)>0){
					$errorArray['username']='Username Already Exist...';
				}
		}
	...
	if(EmptyCheck($dataArray['email'])){
		 $errorArray['email']='Emter Email ID';
				...
				$rs=odbc_exec($conn,$bt);
				if(odbc_num_rows($rs)>0){
					$errorArray['email']='Email ID Already Exist...';
				}
		}
	...
	if(EmptyCheck($dataArray['contact'])){
		...
		$errorArray['contact']='Please Enter A Valid Mobile Number';
				...
				$rs=odbc_exec($conn,$bt);
				if(odbc_num_rows($rs)>0){
					$errorArray['contact']='Contact Number Already Exist...';
				}
		}
...
//print_r($errorArray);
	...
	if(empty($errorArray)){
		...
		if(isset($pid) && $pid>0){
		 ...
		 $upd  ="UPDATE user_management set 
		 user_type='".dbInput($dataArray['user_type'])."', 
		 fullname='".dbInput($dataArray['fullname'])."',  
		 username='".dbInput($dataArray['username'])."', 
		 password='".dbInput($dataArray['password'])."', 
		 email='".dbInput($dataArray['email'])."', 
		 contact='".dbInput($dataArray['contact'])."', 
		 parent_id= '".dbInput($dataArray['parent_id'])."',
		 employee_territory='".dbInput($dataArray['employee_territory'])."',  
		 employee_department='".dbInput($dataArray['employee_department'])."',
		 emp_status='".dbInput($dataArray['emp_status'])."',
		 emp_code='".dbInput($dataArray['emp_code'])."' 
		 ";
		...
		$stmt = odbc_prepare($conn, $upd);
				...
				$insert  ="INSERT INTO user_management (
				user_type, 
				fullname, 
				username, 
				password, 
				...
				'".dbInput($dataArray['fullname'])."', 
				'".dbInput($dataArray['username'])."',	
				'".dbInput($dataArray['password'])."', 
				'".dbInput($dataArray['email'])."', 
				'".dbInput($dataArray['contact'])."',
				'".dbInput($dataArray['parent_id'])."', 
				'".dbInput($timestamp)."' , 
				'".dbInput($dataArray['employee_territory'])."', 
				'".dbInput($dataArray['employee_department'])."',
				'".dbInput($dataArray['emp_code'])."',
				'".dbInput($dataArray['emp_status'])."') ";
    ...
    $("#mt").show();
                                  ...
                                  <div class="control-group" >
                                       ...
                                       <div class="controls" >
                                        ...
                                        <?php $general=(trim($dataArray['user_type'])=='general')?'checked':''; ?>
                                          ...
                                          <label class="radio">
                                          <input type="radio" name="user_type" value="admin" id="a"  <?php echo $admin ?> required/><span>Admin</span>
                                	...
                                	<div class="control-group" id="mt">
                                       ...
                                       <div class="controls">
                                    <input type="text" name="emp_code" class="m-wrap medium"  value="<?php echo $dataArray['emp_code']?>" required/>
                                       </div>
                                    </div> 
                        ...
                        <select data-placeholder="Select Territory" name="employee_territory[]" class="chosen span6" multiple="multiple" tabindex="6" required>
                                  ...
                                  <div class="control-group">
                                       ...
                                       <div class="controls">
                                          ...
                                          <option value="">-Select-</option>
                          <option value="GET" <?php echo ($dataArray['employee_department'])=='GET' ? 'selected' : '' ?>>GET</option>
                          <option value="Retail" <?php echo ($dataArray['employee_department'])=='Retail' ? 'selected' : '' ?>>Retail</option>
                          <option value="PET" <?php echo ($dataArray['employee_department'])=='PET' ? 'selected' : '' ?>>PET</option>
                          <option value="SET" <?php echo ($dataArray['employee_department'])=='SET' ? 'selected' : '' ?>>SET</option>
                          <option value="CTU" <?php echo ($dataArray['employee_department'])=='CTU' ? 'selected' : '' ?>>CTU</option>
           ...
           <input type="password"  name="confirmpassword" class="m-wrap large"  value="<?php echo $dataArray['confirmpassword']?>" required/>
                                    ...
                                    <div class="control-group">
                                       ...
                                       <div class="controls">
                                          <select class="medium m-wrap"  name="emp_status"  required >
                                          <option value="">-Select-</option>
                          <option value="1" <?php echo ($dataArray['emp_status'])=='1' ? 'selected' : '' ?>>Active</option>
                          <option value="0" <?php echo ($dataArray['emp_status'])=='0' ? 'selected' : '' ?>>Inactive</option>
