...
if(isset($_GET['back']) && base64_decode($_GET['check'])=='true' ){
		 ...
		 $ss="select * from user_management u where uid='".dbInput($_GET['ODvalueID'])."' ";
}else{
		 $ss="select * from user_management u where uid='".dbInput($_GET['valueID'])."' ";
}
...
//			print_r($ckaf);
			$_SESSION['emp_cka_mapping'] = 		trim($emp_cka_mapping);
	 		...
	 		echo $vckam="select * from user_management where parent_id='".trim($f['uid'])."' ";
...
//echo '<br><br><br>hello</br>';
	$gpsq="select uid from user_management where employee_department='GET' and uid <> '2015' ";
	$gpscnt=0;
...
//echo '<br><br><br>hello</br>';
	$gpsq="select uid from user_management where employee_department='PET' and uid <> '2006' ";
	$gpscnt=0;
	$gpsexe=odbc_exec($conn, $gpsq);
	while($fr=odbc_fetch_array($gpsexe)){
		//$gpscnt=odbc_num_rows($fr);
...
//echo '<br><br><br>hello</br>';
	$gpsq="select uid from user_management where employee_department='SET' and uid <> '2113' ";
	$gpscnt=0;
	$gpsexe=odbc_exec($conn, $gpsq);
	while($fr=odbc_fetch_array($gpsexe)){
		//$gpscnt=odbc_num_rows($fr);
...
//echo '<br><br><br>hello</br>';
	$gpsq="select uid from user_management where employee_department='CTU' and uid <> '2116' ";
	$gpscnt=0;
	$gpsexe=odbc_exec($conn, $gpsq);
	while($fr=odbc_fetch_array($gpsexe)){
		//$gpscnt=odbc_num_rows($fr);
