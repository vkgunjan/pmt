					...
					<div class="span12">
						...
						<div class="portlet box blue">
							<div class="portlet-title">
								<h4><i class="icon-edit"></i>Select Asset To Request for Breakdown Maintenance</h4>
							</div>
...
';
									...
									echo '<td>';
										...
										$href='breakdown-request-submit.php';
										}else{
...
print '
 <table class="table  table-bordered" >
									<thead>
										...
										<tr>
			<th colspan="12" align="center" bgcolor="#F7060D"><font size="4" color=white><center>Your Breakdown Request List</center></font></th>
										</tr>
...
';
 ...
 $sss="SELECT a. [asset_id]
	 ...
	 a.asset_kept_area
	  ...
	  ,b.[request_status_for_wo]
       from breakdown_request b
	   left join asset_master a on b.asset_id = a.asset_id
	   left join asset_type_master atm on a.asset_type=atm.asset_type_id
	   left join plant_building_master pbm on a.plant_building = pbm.plant_building_id
	   left join department_section_master dsm on a.department_section = dsm.department_section_id
	   left join location_master lm on a.asset_location = lm.location_master_id
	   where b.request_generated_by = ".$_SESSION['uid']."
		and b.factory_id=".$_SESSION['factory-id']." 
		order by b.request_id desc 
		";
		
										$rrs=odbc_exec($conn,$sss);
									while($ff = odbc_fetch_array($rrs)){
										print_r($f);
										echo '<tr>';
										echo '<td>'.$count.'</td>';
										echo '<td>'.$ff['asset_code'].'</td>';
										echo '<td>'.$ff['asset_type'].'</td>';										
										echo '<td>'.$ff['asset_name'].'</td>';																
										echo '<td>'.$ff['plant_building_name'].'</td>';	
										echo '<td>'.$ff['department_section_name'].'</td>';
										echo '<td>'.$ff['location_name'].'</td>';
										echo '<td>'.$ff['asset_kept_area'].'</td>';
										echo '<td>'.$ff['model_number'].'</td>';
										echo '<td>'.$ff['serial_number'].'</td>';
										//echo '<td>'.ucfirst($ff['asset_condition']).'</td>';
										echo '</tr>';
										
										echo '<tr>';
										echo '<th colspan="2">Requect Descritioin</th>';
										echo '<td colspan="6">'.$ff['request_description'].'</td>';
											if($ff['request_status_for_wo']=='generated'){
							echo '<td colspan="2" bgcolor="#24AC03"><font size="3" color="white">Status: Work Order Generated</font></td>';												
											}else{
								echo '<td colspan="2" bgcolor="#ECA50A"><font size="3" color="white">Status: Request Under Process</font></td>';												
											}
