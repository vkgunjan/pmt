...
//action plan_project_session_id
	$_SESSION['action_plan_opp_id']=$_REQUEST['pid'];	
		...
		$dataArray=array(
					...
					'contractor_name'			=>	trim(dbOutput($f['contractor_name'])),
...
if(isset($_POST['opportunity'])){
		...
		$dataArray=array(
					...
					'contractor_name'			=>	trim(dbOutput($_POST['contractor_name'])),
...
//print_r($errorArray);
	...
	if(empty($errorArray)){
		...
		if(isset($pid) && $pid>0){
		 ...
		 contractor_name='".dbInput($dataArray['contractor_name'])."',
...
if(!empty($dataArray['gps_department_name'])){
	$upd .=" , gps_department_name='".dbInput($dataArray['gps_department_name'])."' ";
		...
		$stmt = odbc_prepare($conn, $upd);
				...
				'".dbInput($dataArray['contractor_name'])."',
...
<?php } ?>
 ...
 <option value="Not Approved" <?php echo ($dataArray['obl_approval_status'])=='Not Approved' ? 'selected' : '' ?>>Not Approved</option>
  ...
  <select data-placeholder="Select Channel Partner" class="chosen-with-diselect span6" tabindex="-1" id="partner" name="partner" style="width:220px;">

                                          <option value="">-Select-</option>
                                            <?php

										$sql="select * from channel_partner";

									$rs=odbc_exec($conn,$sql);
										while($f = odbc_fetch_array($rs)){
									$selected=($f['partner_id']==$dataArray['partner_name'])?'selected':'';
									echo '<option value="'.$f['partner_id'].'"'.$selected.'>'.$f['partner_name'].'</option>';
										}
									?>
                                          </select>
                                       </div>
                                    ...
                                    <div class="control-group">
                                       ...
                                       <div class="controls">
				   <input type="text"  class="m-wrap large name=" name="contractor_name" value="<?php echo $dataArray['contractor_name']?>" />
                                       </div>
                                    </div>
