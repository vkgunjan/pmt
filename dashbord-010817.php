...
<?php
			if(trim($_SESSION['user_type'])=='management') {
			$bq="SELECT  COUNT(*) as totalb,  sum(obl_sale_forecast_inr) as project  FROM opportunity where [cka_type_id]='1' and [status]='open' ";

			}else{
			$bq="SELECT  COUNT(*) as totalb,  sum(obl_sale_forecast_inr) as project  FROM opportunity where [cka_type_id]='1' and [status]='open'  and created_by='".$_SESSION['uid']."' ";
			}
			...
			//$bpercent=($bf['totalb']/$tot['tlead'])*100;
						$bpercent=32;
?>
<!--BUILDER Lead in pipeline ENDS-->

												<input class="knobify" data-width="115" data-fgcolor="#4b8df8" data-thickness=".2" data-skin="tron" data-displayprevious="true" value="+<?php echo round($bpercent)?>" data-max="100" data-min="-100" title="<?php echo $bpercent?> % of Builder Lead in Pipeline"/>
											</div>
											<div class="details">
						<div class="title">Monthly <i class="<?php echo($bpercent<30)?'icon-caret-down down' :'icon-caret-up' ?>"></i></div>
...
<?php
			if(trim($_SESSION['user_type'])=='management') {
			$bwq="SELECT  COUNT(*) as tw FROM opportunity where [cka_type_id]='1' and [current_stage]='7' ";
			}else{
...
<?php
			if(trim($_SESSION['user_type'])=='management') {
			$blq="SELECT  COUNT(*) as tl FROM opportunity where [cka_type_id]='1' and [status]='lost' ";
			}else{
...
<?php
			if(trim($_SESSION['user_type'])=='management') {
			$bq="SELECT  COUNT(*) as totalb,  sum(obl_sale_forecast_inr) as project  FROM opportunity where [cka_type_id]='2' and [status]='open' ";
			}else{
				$bq="SELECT  COUNT(*) as totalb,  sum(obl_sale_forecast_inr) as project  FROM opportunity where [cka_type_id]='2' and [status]='open'  and created_by='".$_SESSION['uid']."' ";
			}
			$bqe=odbc_exec($conn,$bq);
			$bf=odbc_fetch_array($bqe);
			//print_r($bf);
...
<!--CONTRACTOR Lead in pipeline ENDS-->
												...
												<input class="knobify" data-width="115" data-fgcolor="#852b99" data-thickness=".2" data-skin="tron" data-displayprevious="true" value="+64" data-max="100" data-min="-100" title="<?php echo $bpercent?> % of Contractor Lead in Pipeline"/>
											</div>
											<div class="details">
				<div class="title">Quarterly <i class="<?php echo($bpercent<30)?'icon-caret-down down' :'icon-caret-up' ?>"></i></div>
				<div class="number">&#8377 <?php echo valchar($bf['project']);?></div>
...
<?php
			if(trim($_SESSION['user_type'])=='management') {
			$bwq="SELECT  COUNT(*) as tw FROM opportunity where [cka_type_id]='2' and [current_stage]='7' ";
			}else{
...
<?php
			if(trim($_SESSION['user_type'])=='management') {
			$blq="SELECT  COUNT(*) as tl FROM opportunity where [cka_type_id]='2' and [status]='lost'  ";
			}else{
...
<?php
		if(trim($_SESSION['user_type'])=='management') {
			$bq="SELECT  COUNT(*) as totalb,  sum(obl_sale_forecast_inr) as project  FROM opportunity where [cka_type_id]='3' and [status]='open'  ";
		}else{
			$bq="SELECT  COUNT(*) as totalb,  sum(obl_sale_forecast_inr) as project  FROM opportunity where [cka_type_id]='3' and [status]='open'  and created_by='".$_SESSION['uid']."' ";
		}
			$bqe=odbc_exec($conn,$bq);
			$bf=odbc_fetch_array($bqe);
			//print_r($bf);
...
<!--Govt. Body Lead in pipeline ENDS-->
												...
												<input class="knobify" data-width="115" data-fgcolor="#987B10" data-thickness=".2" data-skin="tron" data-displayprevious="true" value="+1<?php echo $bpercent?>" data-max="200" data-min="-100" title="<?php echo $bpercent?> % of Govt. Body Lead in Pipeline"/>
											</div>
											<div class="details">
					<div class="title">YTM Vs YTD <i class="<?php echo($bpercent<30)?'icon-caret-down down' :'icon-caret-up' ?>"></i></div>
					<div class="number">&#8377 <?php echo valchar($bf['project']);?></div>
...
<?php
		if(trim($_SESSION['user_type'])=='management') {
			$bwq="SELECT  COUNT(*) as tw FROM opportunity where [cka_type_id]='3' and [current_stage]='7'  ";
		}else{
...
<?php
			if(trim($_SESSION['user_type'])=='management') {
			$blq="SELECT  COUNT(*) as tl FROM opportunity where [cka_type_id]='3' and [status]='lost'  ";
			}else{
...
<?php
			if(trim($_SESSION['user_type'])=='management') {
			$bwq="SELECT  COUNT(*) as tw, sum(won_po_value) as ach FROM opportunity where [current_stage]='7' ";
			}else{
 ...
 <!--Govt. Body WON Lead in pipeline ENDS-->
									...
									<div class="span3 responsive" data-tablet="span6" data-desktop="span3">
										<div class="circle-stat block">
											...
											<div class="details">
												<div class="title">Annual Vs YTD <i class="icon-caret-up"></i></div>
...
<?php
			if(trim($_SESSION['user_type'])=='management') {
			$blq="SELECT  COUNT(*) as tl FROM opportunity where [status]='lost'  ";
			}else{
