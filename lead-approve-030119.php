...
if(isset($_POST['approve'])){
	...
	if ($lead_no == $lead_id && $approval_code == $code) {
		$query = "UPDATE opportunity set lead_approval_status = '0', lead_code = '0', current_stage = '2', lead_approval_remark = '$remarks' where lead_id = '$lead_id'";
		$query_result = odbc_exec($conn, $query);


    $sql = "SELECT o.lead_id, u.fullname, o.created_by, o.project_name, m.email, o.project_name  from opportunity o
    inner join user_management u on u.uid = o.created_by
    inner join user_management m on m.uid = o.created_by where o.lead_id = '$lead_id'";
...
if(isset($_POST['reject'])){
	...
	if ($lead_no == $lead_id && $approval_code == $code) {
		$query = "UPDATE opportunity set lead_approval_status = '2', lead_code = '0', lead_reject_remark = '$remarks' where lead_id = '$lead_id'";
		$query_result = odbc_exec($conn, $query);
