...
// Date checking
	...
	if($dataArray['status']=='close' && empty($dataArray['pda_remark'])){
	
		$errorArray['pda_remark']='Error: Please Enter Product Approval Remarks';
	}
...
if(empty($errorArray)) {
	 ...
	 if(isset($pid) && $pid>0){
		 ...
		 if($dataArray['status'] =='close'){
					 $upd2.=" current_stage='5' , product_approval_date='".dbInput($timestamp)."', pda_remark= '".dbInput($dataArray['pda_remark'])."' ";
				}
		...
		$stmt = odbc_prepare($conn, $upd2);
				if (odbc_execute($stmt)){ 
					$msgTxt = ' Product Approval has been Updated Successfully.';
					$msgType = 1;
				}else{
					$msgTxt = 'Sorry! Unable To Update Product Approval Information , Please Try Later.';
					$msgType = 2;
				}
		}
...
//echo 'tcat-'.$tcat;
	...
	if($skucheck+$tcat>=1){
		...
		//print_r($_POST);
			...
			foreach($_POST['sampling'] as $sd){
				...
				$_POST['tile_category'][$sd];
				 ...
				 $upd="update fld set approved='yes', 
					  approved_tile_name='".$_POST['no_of_sku'][$sd]."' 
					  where fld_id='".$sd."' ";
					
					$stmt = odbc_prepare($conn, $upd);

				if (odbc_execute($stmt)){ 
					$msgTxt = ' Approved Tile Details Has Been Updated Successfully.';
					$msgType = 1;
				}else{
					$msgTxt = 'Sorry! Unable To Update Approved Tile Details, Please Try Later.';
					$msgType = 2;
					$fchk++;
				}
			}// main foreach ends
				...
				echo $upd1= "update opportunity set current_stage='5', 
		...
		//updation process ends
			...
			header('Location:product-approval.php?msgType=' . $msgType . '&msgTxt=' . base64_encode($msgTxt));
			exit;
...
function submitform()
{
 ...
 var checkboxs=document.getElementsByName("sampling[]");
   ...
   if(okay > 0)
	  ...
	  alert("Please Select The Approved Tiles Size.");
                  ...
                  <div class="portlet box blue tabbable">
                     <div class="portlet-title">
                        <h4>
                           ...
                           <span class="hidden-480">Product Approval</span>
                           &nbsp;
                        </h4>
                     </div>
...
<?php */?>                      <li><a href="#portlet_tab2" id="tab2" data-toggle="tab">Project Action Plan</a></li>
								<li class="active"><a href="#portlet_tab1" data-toggle="tab">Product Approval</a></li>			
<?php /*?>
                                       ...
                                       <div class="controls">
    									...
    									<?php $lost=($dataArray['status']=='lost')?'checked':''; ?>
                                         ...
                                         <label class="radio">
                                          ...
                                          <span style="font-weight:bold; color:#0C3">Move to Quotation</span>
                                          </label>
         ...
         <tr align="left" height="60" id="q">
						...
						&nbsp;	&nbsp;
						 ...
						 <br/>
							...
							<b>Product Approval Remarks:</b>
								<textarea style="width:400px;; height:20px;" id="pda_remark" name="pda_remark"></textarea>
                        <div style="color:#F00; font-size:15px; text-align:left"><b><br /><?php echo $errorArray['pda_remark']?></b><br /><br /></div>
                            </td>
                        </tr>
			...
			<th colspan="11" style="background-color:#FF9900; font:16px Verdana, Geneva, sans-serif; color:#FFFFFF">Requirements Details </th>
								...
								echo '<td>'.strtoupper($f['to_meet_with']).'</td>';
								echo '<td>'.strtoupper($f['contact_no']).'</td>';
				 ...
				 echo "<td><input type=text name=no_of_sku[$av] style=width:200px;text-transform:uppercase;border-color:$c; value=$nsku></td>";
										}
									}

								}else{
									echo "<td><input type=text name=no_of_sku[$av] style=width:200px;text-transform:uppercase;></td>";
								}
                           }

if(isset($skucheck) && $skucheck>0){
			$ms='Please Enter Approved Tile Name';
                                ...
                                ?>
			                     ...
			                     <!--   <a class="btn blue" href="javascript: submitform()"> <i class="icon-ok"></i> Submit</a>
									...
									<br>
                                       ...
                                       <a href="product-approval.php"><input type="button" value= "Back" ></a>
