<?php 
   ...
   if(isset($_GET['pid']) && isset($_GET['s_del_id'])){
   	$del="delete from schedule where schedule_id='".$_GET['s_del_id']."' ";
  	$e=odbc_exec($conn,$del);
	
	$del1="delete from schedule_generated where schedule_id='".$_GET['s_del_id']."' ";
  	$e1=odbc_exec($conn,$del1);
...
var checkboxs=document.getElementsByName("checklist[]");
 ...
 left join maintenance_type_master m on m.maintenance_type_id=s.maintenance_type_id 
	 ...
	 ";
					...
					echo '<td><a href="schedule-maintenance.php?pid='.$_REQUEST['pid'].'&s_del_id='.$f['schedule_id'].'" onclick="javascript:return confirm(\'Are you sure you want to delete this schedule?\')" class="btn mini red"><i class="icon-trash" ></i> Delete Schedule</a></td>';
	...
	echo '<td width="14%"><a href="edit-schedule-maintenance.php?pid='.$_REQUEST['pid'].'&s_id='.$f['schedule_id'].'" class="btn mini blue"><i class="icon-refresh"></i> Update Checklist</a></td>';
