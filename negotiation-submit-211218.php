	...
	if($skucheck+$tcat>=1){
		...
		//print_r($_POST);
			...
			foreach($_POST['sampling'] as $sd){
				...
				$p = "update fld set final_bid_price = '".$_POST['no_of_sku'][$sd]."', quotation_price = '".$_POST['no_of_sku'][$sd]."', status = '1', quotation_approval = 'No' where fld_id = '".$sd."'";
				...
				if ($stmt_ex){
					...
					//query for email
						$subject = "PMT || Lead Update-[$lead_no]";
                        $to = $m_email;
                        $body = '<html>
							        <head>
							        <title></title>
						...
						<p>This is an auto generated Email. Please do not reply on this. Kindly Approve/Reject Quotation Price.</p>
						...
						$quotation_price = $m['quotation_price'];
...
*/
		                ...
		                /*$nprice="SELECT top 1 negotiation_price from negotiation where fld_id='".$m['fld_id']."' order by negotiation_last_updated desc";	
		                $ars=odbc_exec($conn,$nprice);
						$np = odbc_fetch_array($ars);
						//$n_price = $np['negotiation_price'];

						if(!empty($np['negotiation_price'])){

						$body .= " <td>{$np['negotiation_price']}</td>";
						...
						<p><a href=\"http://localhost:8080/vikash/pmt/price-approve.php?ld=$lead_no&code=$code\">Click Here</a> to Approve the Quotation for futher operation.</p>
						<br><br><br><br><br>
