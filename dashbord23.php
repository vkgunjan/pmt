...
//vks ending
					...
					//echo '<br>';
						...
						//echo $hxx['no'];
							...
							if(trim($hxx['current_stage'])=='Opportunity'){
								$oppval=$hxx['project'];
								$oppno=$hxx['no'];
								$oad=round($oppval/$oppno);
							}
							
							if(trim($hxx['current_stage'])=='First Discussion'){
								$fldval=$hxx['project'];
								$fldno=$hxx['no'];
							...
							if(trim($hxx['current_stage'])=='Sampling'){
							...
							if(trim($hxx['current_stage'])=='Product Approval'){
							...
							if(trim($hxx['current_stage'])=='Quotation'){
							...
							if(trim($hxx['current_stage'])=='Negotiation'){
	...
	<!-- six months data Start -->
			...
			<?php 
			$sqlmonth_old = "SELECT * FROM (SELECT p.stage_name,YEAR(added_date) [Year], DATENAME(MONTH, added_date) [Month], MONTH(added_date)[Months], COUNT(1) [Sales Count]
      ...
      GROUP BY p.stage_name,YEAR(added_date),MONTH(added_date), DATENAME(MONTH, added_date)) AS MontlySalesData PIVOT( SUM([Sales Count])   
    FOR stage_name IN ([Opportunity],[First Discussion],[Sampling],[Product Approval], [Quotation],[Negotiation],[Won])) AS MNamePivot order by [Months] asc";
		...
		$salesmonth_old = odbc_exec($conn, $sqlmonth_old)
...
if(trim($_SESSION['user_type'])=='management') {
		  $hsqa="SELECT * FROM (SELECT p.stage_name,YEAR(added_date) [Year], DATENAME(MONTH, added_date) [Month], 
				MONTH(added_date)[Months], COUNT(1) [Sales Count]
				FROM opportunity o inner join pmt_current_stage p on o.current_stage=p.stage_id 
				where  added_date > DATEADD(m,-5,getdate()-datepart(d,getdate())+1) and o.status<>'Lost'
				GROUP BY p.stage_name,YEAR(added_date),MONTH(added_date), DATENAME(MONTH, added_date)) AS MontlySalesData PIVOT( SUM([Sales Count])   
				FOR stage_name IN ([Opportunity],[First Discussion],[Sampling],[Product Approval], [Quotation],[Negotiation],[Won])) 
				AS MNamePivot order by [Months] asc";
	}else{
 ...
 ";
    ...
    if(trim($_SESSION['user_type'])=='management') {
		$hsqa.="  where  added_date > DATEADD(m,-5,getdate()-datepart(d,getdate())+1) and o.status<>'Lost'
			...
			if($_SESSION['employee_department']=='Retail'){
				$hsqa.=" where o.deal_type='Retail' 
						and o.territory in (".$_SESSION['employee_territory'].") 
						added_date > DATEADD(m,-5,getdate()-datepart(d,getdate())+1) and o.status<>'Lost'
				GROUP BY p.stage_name,YEAR(added_date),MONTH(added_date), DATENAME(MONTH, added_date)) AS MontlySalesData PIVOT( SUM([Sales Count])   
				FOR stage_name IN ([Opportunity],[First Discussion],[Sampling],[Product Approval], [Quotation],[Negotiation],[Won])) 
				AS MNamePivot order by [Months] asc ";
			}
			...
			if($_SESSION['employee_department']=='CKA'){
					...
					$hsqa.=" where added_date > DATEADD(m,-5,getdate()-datepart(d,getdate())+1) and o.status<>'Lost' 
						...
						$hsqa .=" )GROUP BY p.stage_name,YEAR(added_date),MONTH(added_date), DATENAME(MONTH, added_date)) AS MontlySalesData PIVOT( SUM([Sales Count])   
				FOR stage_name IN ([Opportunity],[First Discussion],[Sampling],[Product Approval], [Quotation],[Negotiation],[Won])) 
				AS MNamePivot order by [Months] asc ";
					}else{
						...
						$hsqa .=" )and o.status <> 'lost' GROUP BY p.stage_name,YEAR(added_date),MONTH(added_date), DATENAME(MONTH, added_date)) AS MontlySalesData PIVOT( SUM([Sales Count])   
				FOR stage_name IN ([Opportunity],[First Discussion],[Sampling],[Product Approval], [Quotation],[Negotiation],[Won])) 
				AS MNamePivot order by [Months] asc";
					}
			...
			if($_SESSION['employee_department']=='BD' || $_SESSION['employee_department']=='GPS'){
					...
					if($_SESSION['user_type']=='manager'){
					$hsqa.=" where o.status<>'lost' and o.state_id is not null 
						and ( ";
				...
				AS MNamePivot order by [Months] asc ";
						...
						$hsqa.="  where o.status<>'lost' and o.state_id is not null and o.created_by='".$_SESSION['uid']."' 
						GROUP BY p.stage_name,YEAR(added_date),MONTH(added_date), DATENAME(MONTH, added_date)) AS MontlySalesData PIVOT( SUM([Sales Count])   
				FOR stage_name IN ([Opportunity],[First Discussion],[Sampling],[Product Approval], [Quotation],[Negotiation],[Won])) 
				AS MNamePivot order by [Months] asc ";	
					}
...
<script type="text/javascript" src="https://www.gstatic.com/charts/loader.js">
    ...
    <script type="text/javascript">
      ...
      function drawChart() {
        ...
        var options = {
          title: 'Pipeline Sales Forecast (INR)',
          pieHole: 0.4,
        };
