...
//vks starting
          ...
          if(trim($_SESSION['user_type'])=='management') {
		  $hsqa="SELECT t.stage_name as opportunityname, ISNULL(COUNT(*),0) as no, sum(o.obl_sale_forecast_inr) as project, sum(o.won_po_value) as won_po_value FROM opportunity o
		...
		//vineet start
		$hsqa="SELECT ISNULL(count(*),0) as no,sum(d.obl_sale_forecast_inr) as project, sum(d.won_po_value) as won_po_value, t.stage_name as opportunityname FROM [opportunity] d
...
//vks ending
					...
					//echo '<br>';
						...
						//echo $hxx['no'];
							...
							if(trim($hxx['opportunityname'])=='Opportunity'){
								$oppval=$hxx['project'];
								$oppno=$hxx['no'];
								$oad=round($oppval/$oppno);
							}
							
							if(trim($hxx['opportunityname'])=='First Discussion'){
								$fldval=$hxx['project'];
								$fldno=$hxx['no'];
							...
							if(trim($hxx['opportunityname'])=='Sampling'){
							...
							if(trim($hxx['opportunityname'])=='Product Approval'){
							...
							if(trim($hxx['opportunityname'])=='Quotation'){
							...
							if(trim($hxx['opportunityname'])=='Negotiation'){
							...
							if(trim($hxx['opportunityname'])=='Won'){
			...
			<?php 
				$zonesql = "select ISNULL(count(*),0) as no, zone  from dashboard_view_tbl 
				where zone != '' and status <> 'lost' and view_data_visible_to='".$_SESSION['uid']."' group by zone";
				$zonetotal = odbc_exec($conn, $zonesql);
			
			?>
			
			<!-- Query for Pipeline 2017 -->
          ...
          if(trim($_SESSION['user_type'])=='management') {
		  $hsqa="select ISNULL(count(*),0) as no, t.stage_name as opportunityname from opportunity d
					left join pmt_current_stage t on t.stage_id = d.current_stage
					where year(d.added_date) = '2017' and d.status <> 'lost' and d.state_id is not null group by t.stage_name";
	}else{
    ...
    if(trim($_SESSION['user_type'])=='management') {
		$hsqa.="  where d.status<>'lost' and d.state_id is not null and year(d.added_date) = '2017' group by t.stage_name";
	}else{

			/*if($_SESSION['employee_department']=='Retail'){
				$hsqa.=" where d.deal_type='Retail' and year(d.added_date) = '2017' and d.state_id is not null
						and d.territory in (".$_SESSION['employee_territory'].") 
						group by t.stage_name ";
			}
			...
			if($_SESSION['employee_department']=='CKA'){
					...
					$hsqa.=" where d.status<>'lost' and year(d.added_date) = '2017' and d.state_id is not null
						and ( ";
						...
						$hsqa .=" )and year(d.added_date) = '2017' and d.state_id is not null and d.status<>'lost' group by t.stage_name ";
					}
			...
			if($_SESSION['employee_department']=='GET' || $_SESSION['employee_department']=='PET' || $_SESSION['employee_department']=='SET' || $_SESSION['employee_department']=='CTU'|| $_SESSION['employee_department']=='Retail'){
					...
					$hsqa.=" where d.status<>'lost' and year(d.added_date) = '2017' and d.state_id is not null 
						...
						$hsqa.="  where d.created_by='".$_SESSION['uid']."' and year(d.added_date) = '2017' and d.status<>'lost' and d.state_id is not null 
						group by t.stage_name ";	
					}
		  ...
		  $hsqa="select ISNULL(count(*),0) as no, t.stage_name as opportunityname from opportunity d
					left join pmt_current_stage t on t.stage_id = d.current_stage
					where year(d.added_date) = '2018' and d.status <> 'lost' and d.state_id is not null group by t.stage_name";
	}else{
    ...
    if(trim($_SESSION['user_type'])=='management') {
		$hsqa.="  where d.status<>'lost' and d.state_id is not null and year(d.added_date) = '2018' group by t.stage_name";
	}else{

			/*if($_SESSION['employee_department']=='Retail'){
				$hsqa.=" where d.deal_type='Retail' and year(d.added_date) = '2018' and d.state_id is not null
						and d.territory in (".$_SESSION['employee_territory'].") 
						group by t.stage_name ";
			}
			...
			if($_SESSION['employee_department']=='CKA'){
					...
					$hsqa.=" where d.status<>'lost' and year(d.added_date) = '2018' and d.state_id is not null
						and ( ";
						...
						$hsqa .=" )and year(d.added_date) = '2018' and d.state_id is not null and d.status<>'lost' group by t.stage_name ";
					}
			...
			if($_SESSION['employee_department']=='GET' || $_SESSION['employee_department']=='PET' || $_SESSION['employee_department']=='SET' || $_SESSION['employee_department']=='CTU'|| $_SESSION['employee_department']=='Retail'){
					...
					$hsqa.=" where d.status<>'lost' and year(d.added_date) = '2018' and d.state_id is not null 
						...
						$hsqa.="  where d.created_by='".$_SESSION['uid']."' and year(d.added_date) = '2018' and d.status<>'lost' and d.state_id is not null 
						group by t.stage_name ";	
					}
					...
					$yearcurrent=odbc_exec($conn,$hsqa);
					
			?>

	<!-- six months data Start -->
			
			
		
	<!-- six months data End -->

<!-- Barchart Query Starts -->
<?php

if(trim($_SESSION['user_type'])=='management') {
		  $hsqa="SELECT [Year],[Month],[Months],ISNULL([Opportunity],0)[Opportunity],ISNULL([First Discussion],0)[First Discussion], 
				ISNULL([Sampling],0)[Sampling],ISNULL([Product Approval],0)[Product Approval],ISNULL([Quotation],0)[Quotation],ISNULL([Negotiation],0)[Negotiation],ISNULL([Won],0)[Won]
				FROM (
				SELECT p.stage_name,YEAR(added_date) [Year], DATENAME(MONTH, added_date) [Month], MONTH(added_date)[Months], COUNT(1) [Sales Count]
				FROM opportunity o inner join pmt_current_stage p on o.current_stage=p.stage_id where o.status<>'lost' and  added_date > DATEADD(m,-5,getdate()-datepart(d,getdate())+1)
				GROUP BY p.stage_name,YEAR(added_date),MONTH(added_date), DATENAME(MONTH, added_date)
				) AS MontlySalesData PIVOT( SUM([Sales Count])  
				FOR stage_name IN ([Opportunity],[First Discussion],[Sampling],[Product Approval], [Quotation],[Negotiation],[Won])) AS MNamePivot order by [Months] asc";
	}else{
		...
		$hsqa="SELECT [Year],[Month],[Months],ISNULL([Opportunity],0)[Opportunity],ISNULL([First Discussion],0)[First Discussion], 
				...
				FROM (
				SELECT p.stage_name,YEAR(added_date) [Year], DATENAME(MONTH, added_date) [Month], MONTH(added_date)[Months], COUNT(1) [Sales Count]
				FROM opportunity o inner join pmt_current_stage p on o.current_stage=p.stage_id
				left join cka_name_master a on a.cka_name_id = o.cka_name_id
				 
 ";


    if(trim($_SESSION['user_type'])=='management') {
		$hsqa.="  where o.status<>'lost' and  added_date > DATEADD(m,-5,getdate()-datepart(d,getdate())+1)
					GROUP BY p.stage_name,YEAR(added_date),MONTH(added_date), DATENAME(MONTH, added_date)
					) AS MontlySalesData PIVOT( SUM([Sales Count]) FOR stage_name IN ([Opportunity],[First Discussion],[Sampling],[Product Approval], [Quotation],[Negotiation],[Won])) AS MNamePivot order by [Months] asc";
	}else{

			if($_SESSION['employee_department']=='Retails'){
				$hsqa.=" where o.deal_type='Retails' 
						and o.territory in (".$_SESSION['employee_territory'].") and 
						 o.status<>'Lost' and 
				added_date > DATEADD(m,-5,getdate()-datepart(d,getdate())+1)
					GROUP BY p.stage_name,YEAR(added_date),MONTH(added_date), DATENAME(MONTH, added_date)
					) AS MontlySalesData PIVOT( SUM([Sales Count]) FOR stage_name IN ([Opportunity],[First Discussion],[Sampling],[Product Approval], [Quotation],[Negotiation],[Won])) AS MNamePivot order by [Months] asc";
			}
			...
			if($_SESSION['employee_department']=='CKA'){
					...
					$hsqa.=" where added_date > DATEADD(m,-5,getdate()-datepart(d,getdate())+1) and o.status<>'Lost' 
						and ( ";
						...
						$hsqa .=" )GROUP BY p.stage_name,YEAR(added_date),MONTH(added_date), DATENAME(MONTH, added_date)
					) AS MontlySalesData PIVOT( SUM([Sales Count]) FOR stage_name IN ([Opportunity],[First Discussion],[Sampling],[Product Approval], [Quotation],[Negotiation],[Won])) AS MNamePivot order by [Months] asc";
					}else{
						$hsqa.=" where ( o.deal_type='CKA') and ( ";
						...
						//echo $vx;
							if($xcnt==0)
								$hsqa .="o.cka_name_id = '".$vx."' ";
							else
								$hsqa .=" or o.cka_name_id = '".$vx."' ";
							$xcnt++;
						}

						$hsqa .=" )and added_date > DATEADD(m,-5,getdate()-datepart(d,getdate())+1) and o.status <> 'lost' GROUP BY p.stage_name,YEAR(added_date),MONTH(added_date), DATENAME(MONTH, added_date)
					) AS MontlySalesData PIVOT( SUM([Sales Count]) FOR stage_name IN ([Opportunity],[First Discussion],[Sampling],[Product Approval], [Quotation],[Negotiation],[Won])) AS MNamePivot order by [Months] asc";
					}
			...
			if($_SESSION['employee_department']=='GET' || $_SESSION['employee_department']=='PET' || $_SESSION['employee_department']=='SET' || $_SESSION['employee_department']=='CTU'|| $_SESSION['employee_department']=='Retail'){
					...
					if($_SESSION['user_type']=='manager'){
					$hsqa.=" where added_date > DATEADD(m,-5,getdate()-datepart(d,getdate())+1) and o.status<>'Lost' and o.state_id is not null 
						and ( ";
						...
						//echo $vx;
							if($xcnt==0)
								$hsqa .="o.created_by = '".$vx."' or o.created_by='".$_SESSION['uid']."' ";
							else
								$hsqa .=" or o.created_by = '".$vx."' ";
							$xcnt++;
						}
						$hsqa .=" )GROUP BY p.stage_name,YEAR(added_date),MONTH(added_date), DATENAME(MONTH, added_date)
					) AS MontlySalesData PIVOT( SUM([Sales Count]) FOR stage_name IN ([Opportunity],[First Discussion],[Sampling],[Product Approval], [Quotation],[Negotiation],[Won])) AS MNamePivot order by [Months] asc ";
					}else{
						$hsqa.=" where added_date > DATEADD(m,-5,getdate()-datepart(d,getdate())+1) and o.status<>'Lost' and o.state_id is not null and o.created_by='".$_SESSION['uid']."' 
						GROUP BY p.stage_name,YEAR(added_date),MONTH(added_date), DATENAME(MONTH, added_date)
					) AS MontlySalesData PIVOT( SUM([Sales Count]) FOR stage_name IN ([Opportunity],[First Discussion],[Sampling],[Product Approval], [Quotation],[Negotiation],[Won])) AS MNamePivot order by [Months] asc ";	
					}
		...
		//vineet ends
			...
			$salesmonth = odbc_exec($conn, $hsqa);
	?>
...
<script type="text/javascript" src="https://www.gstatic.com/charts/loader.js">
    ...
    <script type="text/javascript">
      ...
      function drawChart() {
        ...
        var options = {
          title: 'Pipeline Summary (In Numbers)'
        };
	...
	<script type="text/javascript">
      ...
      function drawChart() {
        var data = google.visualization.arrayToDataTable([
		  ...
		  <?php 
			while($hxx2=odbc_fetch_array($zonetotal)){
				echo "['".$hxx2["zone"]."', ".$hxx2["no"]."],";
			}
		  ?>
        ...
        var data = google.visualization.arrayToDataTable([
          ...
          <?php 
			while($hxx1=odbc_fetch_array($heeesum)){
				...
				echo "['".$hxx1["opportunityname"]."', ".$hxx1["project"]."/10000000],";
			}
		  ?>
        ]);

        var options = {
          title: 'Pipeline Sales Forecast (In Crore)',
          pieHole: 0.4,
        };
	...
	<script type="text/javascript">
      ...
      function drawVisualization() {
        ...
        var data = google.visualization.arrayToDataTable([
		 ...
		 <?php 
		 while($hxx7=odbc_fetch_array($salesmonth)){
				
				echo "['".$hxx7["Month"]."', ".$hxx7["Opportunity"].", ".$hxx7["First Discussion"].", ".$hxx7["Sampling"].", ".$hxx7["Product Approval"].", ".$hxx7["Quotation"].", ".$hxx7["Negotiation"].", ".$hxx7["Won"]."],";
	...
	<!-- <script type="text/javascript">
	  ...
	  function drawChart() {
	    var oldData = google.visualization.arrayToDataTable([
	      ['Name', 'Pipeline 2017'],
	      <?php 
			while($hxx3=odbc_fetch_array($yearprevious)){
				
				echo "['".$hxx3["opportunityname"]."', ".$hxx3["no"]."],";
			}
		  ?>
	    ]);
	    ...
	    var newData = google.visualization.arrayToDataTable([
	      ['Name', 'Pipeline 2018'],
	      <?php 
			while($hxx4=odbc_fetch_array($yearcurrent)){
	
				echo "['".$hxx4["opportunityname"]."', ".$hxx4["no"]."],";
			}
		  ?>
	    ]);
	    ...
	    barChartDiff.draw(diffData, options);
			...
			<div class="row-fluid">
				...
				<div  id="piechart" style="width: 35%; height: 300px;  float: left;"></div>
				<div  id="piechart_3d" style="width: 30%; height: 300px; float: left;"></div>
				<div  id="donutchart" style="width: 35%; height: 300px; float: left;"></div>
...
//vks ends
			...
			if($_SESSION['employee_department']=='GET' || $_SESSION['employee_department']=='PET' || $_SESSION['employee_department']=='SET' || $_SESSION['employee_department']=='CTU'){
