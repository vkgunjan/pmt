...
$ex=explode(",",$_POST['cart_value_id']);
foreach($ex as $x){
	 $action_status=$x.'action_status';
 	 $remarks=$x.'remarks';
	// $_POST[$action_status][0];
	// $_POST[$action_remarks][0];
//echo $x;
			if($_POST[$action_status][0]=='Accept'){
				$val=1;
			}

			if($_POST[$action_status][0]=='Reject'){
				$val=2;
			}


			if($_POST[$action_status][0]=='Done'){
				$val=3;
			}

			
			if($val>0){
					echo $upd="update project_action_plan_cart set action_status = '".$val."', action_remarks = '".$_POST[$remarks][0]."' 
					where action_plan_cart_id = '".$x."' ";
		
						$stmt1 = odbc_prepare($conn, $upd);

						if (odbc_execute($stmt1)){ 
							$msgTxt = ' Action Plan Details Has Been Updated Successfully.';
							$msgType = 1;
						}else{
							$msgTxt = 'Sorry! Unable To Update Action Plan Details, Please Try Later.';
							$msgType = 2;
						}

						header('Location:list-dependency-lead.php?msgType=' . $msgType . '&msgTxt=' . base64_encode($msgTxt));
							exit;
			}
}
                  ...
                  <div class="portlet box blue tabbable">
                     <div class="portlet-title">
                        <h4>
                           ...
                           <span class="hidden-480">Lead Support Required</span>
                           &nbsp;
                        </h4>
                     </div>
...
<?php */?>                      
								<li class="active"><a href="#portlet_tab1" data-toggle="tab">Lead Support Require</a></li>			
<?php /*?>
       ...
       <?php
		...
		";
                             ...
                             <table border="1" style="width:100%" align="center">
                              	...
                              	<tr height="25">
			<th colspan="8" style="background-color:#FF9900; font:16px Verdana, Geneva, sans-serif; color:#FFFFFF"> Required Support Details </th>
								</tr>

								<tr>
                                    <th width="10%">Activity</th>
                                    <th width="5%">Responsible Department</th>
                                    <th width="5%">Responsible Owner</th>
                                    <th width="5%">Timeline</th>
                                    <th width="5%">Request By</th>
									<th width="15%">Request Remarks</th>
                                ...
                                //echo 'hello';
                                //print_r($_GET);
								  $ssql="select * from project_action_plan_cart where opportunity_id='".$_REQUEST['pid']."' 
								  and action_status='0' or action_status='1'  ";	
								...
								if($cnt==0){
									$cart_value_id =$f['action_plan_cart_id'];
								}else{
									$cart_value_id .=','.$f['action_plan_cart_id'];
								}
									$cnt++;	
								...
								echo '<td>'.$f['request_remarks'].'</td>';
								
								//echo gettype($f['action_status']);
							...
							//echo $sd;
								echo '<td align="left">
								  ...
								  <label class="radio">
				 <input type="radio" name="'.$f['action_plan_cart_id'].'action_status[]" value="Accept" '.$as.'/>Accept
								  </label>

								  <label class="radio">
				 <input type="radio" name="'.$f['action_plan_cart_id'].'action_status[]" value="Reject" />Reject
								  </label>
								  
								  <label class="radio">
				<input type="radio" name="'.$f['action_plan_cart_id'].'action_status[]" value="Done" />Done 
								  </label>
										  
								</td>';
							
									echo '<td><textarea name="'.$f['action_plan_cart_id'].'remarks[]">'.$f['action_remarks'].'</textarea></td>';
                                ...
                                ?>
                                   ...
                                   <input type="hidden" name="cart_value_id" value="<?php echo $cart_value_id?>" />
                                    <input type="hidden" name="pid" value="<?php echo $_REQUEST['pid']?>" />
                                    
			                        <a class="btn blue" href="javascript: submitform()"> <i class="icon-ok"></i> Submit</a>
                 
                                       <a href="list-dependency-lead.php"><button type="button" class="btn">Cancel</button></a>
